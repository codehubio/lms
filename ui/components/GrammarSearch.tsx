...
export default function GrammarSearch() {
  ...
  const handleSearch = useCallback((value: string) => {
    ...
    router.push(`/${locale}/grammar/${entryId}?${params.toString()}`);
  ...
  return (
    <form onSubmit={handleSubmit} className="mb-4 sm:mb-6" suppressHydrationWarning>
      <label htmlFor="grammar-search-input" className="sr-only">
        {text.grammar?.searchPlaceholder || 'Search grammar topics...'}
      </label>
      <input
        ...
        placeholder={text.grammar?.searchPlaceholder || 'Search grammar topics...'}
        aria-label={text.grammar?.searchPlaceholder || 'Search grammar topics...'}
      ...
      <div className="flex gap-2">
        <button
          type="submit"
          aria-label={text.grammar?.searchButton || 'Search'}
