...
export async function fetchDictionaryEntries(options?: {
  ...
  const { page = 1, pageSize = 50, search = '', tag = '', language = 'en' } = options || {};
  ...
  try {
    ...
    // Also find example sentences that contain each word
    ...
    grouped_data AS (
      SELECT 
        ...
        COALESCE(ex.examples, []) as examples
      FROM vocabulary v
      LEFT JOIN aggregated_tags tag_agg ON v.word = tag_agg.word
      LEFT JOIN word_examples ex ON v.word = ex.word
      ${qualifiedWhereClause}
      GROUP BY v.word, tag_agg.tags, ex.examples
    )
    SELECT * FROM (
      SELECT 
