...
This is a **Chinese Language Learning Management System (LMS)** built with modern web technologies. The project provides a multilingual (English/Vietnamese/Chinese) interface for exploring Chinese vocabulary with comprehensive HSK word lists, featuring advanced search, filtering, and pagination capabilities.
...
- Nested translation structure in JSON
- UI text loaded from `constants/ui-text.json`
- Database translations stored as JSON objects with language keys
...
- **Tag Filtering**: Filter by HSK levels and custom tags (OR logic)
